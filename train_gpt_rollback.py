"""
GPT-2 Style Model in Full Competition Pipeline
==============================================
- 99% identical to the official competition train_gpt.py infrastructure
- Architecture: Classic GPT-2 style (absolute pos emb + LayerNorm + standard MHA + GELU + 4x MLP)
- Everything else (data, eval, Muon, EMA, int8+zlib export, etc.) is untouched

Save as: train_gpt2_competition_style.py
...
class Hyperparameters:
    ...
    mlp_mult = int(os.environ.get("MLP_MULT", 4))          # GPT-2 classic
    tie_embeddings = bool(int(os.environ.get("TIE_EMBEDDINGS", "1")))

    # Optimizer (adapted for GPT-2 while keeping Muon infrastructure)
    embed_lr = float(os.environ.get("EMBED_LR", 0.75))
    head_lr = float(os.environ.get("HEAD_LR", 0.01))
    tied_embed_lr = float(os.environ.get("TIED_EMBED_LR", 0.075))
    matrix_lr = float(os.environ.get("MATRIX_LR", 0.065))
    scalar_lr = float(os.environ.get("SCALAR_LR", 0.065))
...
class Muon(torch.optim.Optimizer):
    def __init__(self, params, lr: float, momentum: float, backend_steps: int, nesterov: bool = True):
        super().__init__(
            params, dict(lr=lr, momentum=momentum, backend_steps=backend_steps, nesterov=nesterov)
        )
...
# -----------------------------
# TOKENIZER-AGNOSTIC EVALUATION (unchanged)
# -----------------------------
...
# -----------------------------
# POST-TRAINING QUANTIZATION (unchanged)
# -----------------------------
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    pattern for pattern in os.environ.get("CONTROL_TENSOR_NAME_PATTERNS", "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights").split(",") if pattern
)
...
def main() -> None:
    ...
    log0(f"train_loader:dataset:fineweb10B_sp1024")
    ...
    # Optimizer split
    matrix_params = [p for n, p in base_model.named_parameters() if p.ndim == 2]
    scalar_params = [p for n, p in base_model.named_parameters() if p.ndim < 2]

    token_lr = args.tied_embed_lr if args.tie_embeddings else args.embed_lr

    optimizer_tok = torch.optim.Adam(
        [{"params": [base_model.tok_emb.weight], "lr": token_lr}],
    ...
    if master_process:
        wandb.init(
            ...
            name=f"gpt2_comp_style_{int(time.time())}",
            config={**{k: v for k, v in vars(args).items() if not k.startswith("_")}, "architecture": "gpt2_baseline"},
            save_code=True,
        )
        if hasattr(wandb, "run") and wandb.run is not None:
            wandb.run.log_code()

    log0(f"model_params:{n_params} (GPT-2 style)")
    ...
    while True:
        ...
        frac = min(step / 200, 1.0) if 200 > 0 else 1.0   # simple momentum warmup
        muon_momentum = (1 - frac) * 0.9 + frac * args.muon_momentum
        for group in optimizer_muon.param_groups:
            group["momentum"] = muon_momentum

        for opt in optimizers:
            for group in opt.param_groups:
                group["lr"] = group.get("base_lr", group["lr"]) * scale   # fallback if no base_lr
        ...
        reached_cap = False
        if max_wallclock_ms is not None and (should_log_train or last_step):
    ...
    # Use EMA for final export
    if master_process and ema_model is not None:
        log0("Using EMA weights for final evaluation and export")
    ...
    # Cleanup
    import gc
    del model, base_model, compiled_model, optimizers, train_loader, ema_model
    gc.collect()
