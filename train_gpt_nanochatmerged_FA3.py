...
from flash_attention_local import HAS_FA3
...
class Hyperparameters:
    ...
    # nanochat-style: flat hold after warmup then linear warmdown
    # hold_steps = iterations // 20  (5% of training at peak LR)
    # warmdown_iters controls linear warmdown length at end
    warmdown_iters        = int(os.environ.get("WARMDOWN_ITERS",        2000))   # linear warmdown at end
    train_batch_tokens    = int(os.environ.get("TRAIN_BATCH_TOKENS",    524_288))
    train_seq_len         = int(os.environ.get("TRAIN_SEQ_LEN",         1024))
    max_wallclock_seconds = float(os.environ.get("MAX_WALLCLOCK_SECONDS", 3600))
    ...
    final_lr_frac       = float(os.environ.get("FINAL_LR_FRAC",        0.1))   # min LR = base_lr * final_lr_frac

    # Muon
    muon_momentum              = float(os.environ.get("MUON_MOMENTUM",              0.95))
    muon_backend_steps         = int(os.environ.get("MUON_BACKEND_STEPS",           5))
    # nanochat-style momentum schedule: warmup 0.85→0.97, warmdown 0.97→0.90
    muon_momentum_warmup_start = float(os.environ.get("MUON_MOMENTUM_WARMUP_START", 0.85))
    muon_momentum_peak         = float(os.environ.get("MUON_MOMENTUM_PEAK",         0.97))
    muon_momentum_warmdown_end = float(os.environ.get("MUON_MOMENTUM_WARMDOWN_END", 0.90))
    muon_momentum_warmup_steps = int(os.environ.get("MUON_MOMENTUM_WARMUP_STEPS",   400))
    # Muon weight decay (nanochat: 0.28, cosine-decayed to zero over training)
    muon_weight_decay          = float(os.environ.get("MUON_WEIGHT_DECAY",          0.1))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                # weight decay applied before the orthogonalized update (decoupled)
                if weight_decay != 0.0:
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int,
                 rope_base: float, qk_gain_init: float, use_fa3: bool = False):
        super().__init__()
        if dim % num_heads != 0:
        ...
        # FA3 setup — imported lazily so script runs without flash-attn installed
        if use_fa3:
            try:
                from flash_attn import flash_attn_func
                self._fa3_fn = flash_attn_func
            except ImportError:
                raise ImportError(
                    "USE_FA3=1 requires flash-attn>=2.5 installed. "
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        #     y = self._fa3_fn(q_fa, k_fa, v_fa, causal=True)
        #     y = y.reshape(bsz, seqlen, dim)
        if self.use_fa3:
            # FA3 expects (batch, seqlen, heads, head_dim) and ONLY fp16/bf16
            q_fa = q.transpose(1, 2).to(torch.bfloat16).contiguous()
            k_fa = k.transpose(1, 2).to(torch.bfloat16).contiguous()
            v_fa = v.transpose(1, 2).to(torch.bfloat16).contiguous()

            y = self._fa3_fn(q_fa, k_fa, v_fa, causal=True)

            y = y.to(dtype=q.dtype)  # bring back to original dtype if needed
            y = y.reshape(bsz, seqlen, dim)
        else:
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # relu² from modded-nanogpt — reverted from LeakyReLU² (incorrect squaring behavior)
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int,
        ...
        self.attn      = CausalSelfAttention(dim, num_heads, num_kv_heads,
                                             rope_base, qk_gain_init, use_fa3)
...
class GPT(nn.Module):
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 ...
                 rope_base, qk_gain_init, use_fa3=False):
        super().__init__()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        logits_proj = (F.linear(x, self.tok_emb.weight) if self.tie_embeddings
                       else self.lm_head(x))
...
# -----------------------------
# warmup → flat hold (5% of total) → linear warmdown
# min LR = base_lr * final_lr_frac

def make_lr_mul(args: Hyperparameters):
    hold_steps     = args.iterations // 20                         # 5% flat peak
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        ...
        if step < warmdown_start:
            return 1.0
        # linear warmdown → final_lr_frac
        progress = (step - warmdown_start) / max(args.warmdown_iters, 1)
        progress = min(progress, 1.0)
        return (1.0 - progress) * 1.0 + progress * args.final_lr_frac

    return lr_mul


# nanochat-style Muon momentum schedule:
# 0.85 → 0.97 over first 400 steps, hold at 0.97, then 0.97 → 0.90 during warmdown
def get_muon_momentum(step: int, args: Hyperparameters) -> float:
    warmdown_start = args.iterations - args.warmdown_iters
    if step < args.muon_momentum_warmup_steps:
        frac = step / max(args.muon_momentum_warmup_steps, 1)
        return (1 - frac) * args.muon_momentum_warmup_start + frac * args.muon_momentum_peak
    if step >= warmdown_start:
        progress = (step - warmdown_start) / max(args.warmdown_iters, 1)
        progress = min(progress, 1.0)
        return (1 - progress) * args.muon_momentum_peak + progress * args.muon_momentum_warmdown_end
    return args.muon_momentum_peak


# nanochat-style weight decay schedule: cosine decay from muon_weight_decay → 0
def get_muon_weight_decay(step: int, args: Hyperparameters) -> float:
    return args.muon_weight_decay * 0.5 * (1.0 + math.cos(math.pi * step / max(args.iterations, 1)))
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        qk_gain_init=args.qk_gain_init, use_fa3=args.use_fa3,
    ).to(device).bfloat16()
    ...
    optimizer_muon = Muon(
        ...
        momentum=args.muon_momentum_warmup_start,  # starts at warmup value
        ...
        wandb.init(
            ...
            config={
                **{k: v for k, v in vars(args).items() if not k.startswith("_")},
                ...
                "model_params": n_params, "torch_version": torch.__version__,
            },
            save_code=True,
        )
    ...
    log0(f"lr_schedule: warmup={args.warmup_steps} hold={args.iterations//20} "
         f"warmdown={args.warmdown_iters} final_lr_frac={args.final_lr_frac}")
    log0(f"muon_momentum: {args.muon_momentum_warmup_start}→{args.muon_momentum_peak}"
         f"→{args.muon_momentum_warmdown_end}")
    ...
    # GC freeze after warmup — nanochat trick: prevents Python GC scanning training objects mid-run
    gc.collect()
    gc.freeze()
    gc.disable()
    ...
    while True:
        ...
        # Update Muon momentum and weight decay (nanochat schedules)
        muon_mom = get_muon_momentum(step, args)
        muon_wd  = get_muon_weight_decay(step, args)
        for group in optimizer_muon.param_groups:
        ...
        if should_log_train:
            ...
            if USE_WANDB and master_process:
                log_dict = {
                    "step": step, "train_loss": train_loss.item(),
                    "train_time_ms": approx_ms, "step_avg_ms": approx_ms / step,
                    "lr_scale": scale, "muon_momentum": muon_mom,
    ...
    del model, base_model, compiled_model, optimizers, train_loader
    gc.collect()
